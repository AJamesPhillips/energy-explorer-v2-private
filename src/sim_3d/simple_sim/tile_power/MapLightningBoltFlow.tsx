...
export function MapLightningBoltFlow()
{
    ...
    useEffect(() => pub_sub.sub("power_gen_cap_store_and_demand", ({ gen_cap_store_MW_by_h3r4, demand_GW_by_h3r4 }) =>
    {
        ...
        ids.forEach(h3_id =>
        {
            const generated_gw = (gen_cap_store_MW_by_h3r4[h3_id]?.total_generated_MW ?? 0) / 1000
            const demand_gw = demand_GW_by_h3r4[h3_id]?.demand_GW ?? 0
            if (!generated_gw && !demand_gw) return
            const xy = H3R4_CELL_TO_XY.get(h3_id)
            if (!xy) return
            next.push({ id: h3_id, x: xy.x, y: xy.y, generated_gw, demand_gw })
        })
